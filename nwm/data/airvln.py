...
class BaseDataset(Dataset):
    def __init__(
        ...
        all_data_config = load_yaml("config/data_config.yaml")
    ...
    # ============ seq2seq ============
    def _compute_projected_images(
        self, traj_data, context_times, rgb_seq, goal_times_np
    ):
        """
...
class TrainingDataset(BaseDataset):
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            rel_time = (goal_offset).astype("float") / (128.0)  # TODO: tune this const
            ...
            # aug
            rgb_imgs = [
                cv2.imread(get_data_path(self.data_folder, f_img, t_img))
                for f_img, t_img in true_context
            ]
            rgb_imgs = [
                cv2.cvtColor(rgb_img, cv2.COLOR_BGR2RGB) for rgb_img in rgb_imgs
            ]
            ...
            projected_images = self._compute_projected_images(
                curr_traj_data, context_times, rgb_imgs, goal_time
            )  # (B,H,W,3)
...
class EvalDataset(BaseDataset):
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            pred_times = list(
                range(curr_time + 1, curr_time + self.len_traj_pred + 1)
            )  # 未来 B 帧
            ...
            actions, _ = self._compute_actions(
                curr_traj_data, curr_time, np.array(pred_times)
            )
            ...
            # 多历史帧 → 多目标帧重投影（B 张投影图）
            projected_images = self._compute_projected_images(
                curr_traj_data, context_times, rgb_imgs, np.array(pred_times)
            )
            projected_tensor_list = [
                self.transform(Image.fromarray(img)) for img in projected_images
            ]
...
class TrajectoryEvalDataset(BaseDataset):
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            rgb_imgs = np.stack(rgb_imgs, axis=0)
            projected_images = self._compute_projected_images(
                curr_traj_data, context_times, rgb_imgs, np.array([goal_time])
            )
            projected_tensor_list = [
                self.transform(Image.fromarray(img)) for img in projected_images
            ]
