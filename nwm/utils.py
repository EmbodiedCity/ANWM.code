...
def log_viz_single(
    ...
    #################################
    plot_array = plot_images_and_actions(
        dataset_name,
        viz_obs_image,
        viz_goal_image,
        pred_actions,
...
def plot_images_and_actions(
    dataset_name,
    curr_viz_obs_image,
    curr_viz_goal_image,
    curr_viz_pred_actions,
    curr_viz_actions,
    min_idx,
    loss,
    metric_waypoint_spacing=None,
    ...
    spacing = metric_waypoint_spacing
    if spacing is None:
        spacing = load_yaml("config/data_config.yaml")[dataset_name][
            "metric_waypoint_spacing"
        ]
    curr_viz_pred_actions = curr_viz_pred_actions * spacing
    curr_viz_actions = curr_viz_actions * spacing
    ...
    show_indices = [min_idx.item()] + other_indices[
        :max_vis
    ]  # 最优的 + 最多 4 条其他的
...
def unnormalize_data(ndata, stats):
    ndata = (ndata + 1) / 2
    data = ndata * (stats["max"].to(ndata) - stats["min"].to(ndata)) + stats["min"].to(
        ndata
    )
    return data
...
def angle_difference(theta1, theta2):
    ...
    delta_theta = delta_theta - 2 * np.pi * np.floor(
        (delta_theta + np.pi) / (2 * np.pi)
    )
    return delta_theta
...
def calculate_delta_yaw(unnorm_actions):
    ...
    delta_yaw = torch.cat(
        (torch.zeros(yaw.shape[0], 1, yaw.shape[2]).to(yaw.device), yaw), dim=1
    )
