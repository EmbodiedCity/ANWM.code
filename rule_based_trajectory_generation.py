...
def rule_based_trajectory(start, steps=8, rule_sequence=None):
    # 基于固定规则的轨迹生成
    trajectory = [start]
    ...
    for i in range(steps):
        ...
        action_choice = rule_sequence[i % len(rule_sequence)]
...
def trajectory_generation_rule_based(GT_trajectory, candidate_number=10, dim=3):
    """
    生成多个轨迹的函数
...
if __name__ == "__main__":
    ...
    candidate_trajectoires = trajectory_generation_rule_based(
        GT_traj, 
    ...
    plot_3d_trajectories(
        ...
        candidate_trajectoires, 
        save_path="./plot", 
        filename=f"trajectories_GT_rule_based_{datetime.datetime.now():%m-%d_%H-%M-%S}.png"
    )
