    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, rel_t, aug, camera_mats in loader:
            ...
            with torch.amp.autocast('cuda', enabled=bfloat_enable, dtype=torch.bfloat16):
                ...
                camera_mats_x_all = torch.cat((camera_mats_x_cond, camera_mats_x_start), dim=1)          # [B*num_goals, num_conds+1, 4, 4]
                
                t = torch.randint(0, diffusion.num_timesteps, (x_start.shape[0],), device=device)
                model_kwargs = dict(y=y, x_cond=x_cond, rel_t=rel_t, x_sup=y_cond.squeeze(1), viewmats=camera_mats_x_all)
