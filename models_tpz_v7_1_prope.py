...
# For ablation studies, only use PRoPE attention in the CDiT blocks.
...
# from timm.models.vision_transformer import PatchEmbed, Attention, Mlp   # [REMOVED]
...
class CDiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        ...
        # self.attn = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)  # [REMOVED]
        # [MOD] 用 PRoPE 自注意力（保持 batch_first 接口：x=[B,T,D]）
        self.attn = PropeSelfAttention(                                       # [ADDED]
            hidden_size, num_heads,                                           # [ADDED]
            patches_x=block_kwargs.get("patches_x"),                           # [ADDED]
            patches_y=block_kwargs.get("patches_y"),                           # [ADDED]
            image_size=block_kwargs.get("image_size"),                         # [ADDED]
            qkv_bias=True,                                                    # [ADDED]
        )                                                                      # [ADDED]
        ...
        # self.cttn = nn.MultiheadAttention(hidden_size, num_heads=num_heads, add_bias_kv=True, bias=True, batch_first=True, **block_kwargs)  # [REMOVED]
        # [MOD] 用 PRoPE 交叉注意力（同样 batch_first：query/key/value=[B,T,D]）
        self.cttn = PropeCrossAttention(                                       # [ADDED]
            hidden_size, num_heads,                                            # [ADDED]
            patches_x=block_kwargs.get("patches_x"),                           # [ADDED]
            patches_y=block_kwargs.get("patches_y"),                           # [ADDED]
            image_size=block_kwargs.get("image_size"),                         # [ADDED]
    ...
    # [MOD] forward 新增 viewmats/Ks，并把它们传给注意力模块
    def forward(self, x, c, x_cond, x_encoded, x_cond_encoded, *, viewmats, Ks=None):  # [MOD]
        ...
        # [MOD] 自注意力：约定最后一个相机是 target → 用 viewmats[:, -1:]
        x = x + gate_msa.unsqueeze(1) * self.attn(
            modulate(self.norm1(x), shift_msa, scale_msa),
            viewmats_q=viewmats[:, -1:],                      # [ADDED]
            Ks_q=None if Ks is None else Ks[:, -1:],          # [ADDED]
        )

        # [MOD] 交叉注意力：Q=目标相机（x_encoded），KV=上下文相机（x_cond_encoded）
        ...
        x = x + gate_ca_x.unsqueeze(1) * self.cttn(
            ...
            value=x_cond_norm,
            viewmats_q=viewmats[:, -1:],                      # [ADDED]
            viewmats_kv=viewmats[:, :-1],                     # [ADDED]
            Ks_q=None if Ks is None else Ks[:, -1:],          # [ADDED]
            Ks_kv=None if Ks is None else Ks[:, :-1],         # [ADDED]
        )
...
# class PRoPEEncoder(nn.Module):
#     ...  # [REMOVED] 不再在 block 外单独封装，直接使用上面的注意力封装
...
class CDiT(nn.Module):
    ...
    def __init__(
        ...
        # [MOD] 保留你原先的 q/k/v 投影（如果你之后不需要“全局一次 PRoPE 编码”，可删）
        self.q_proj = nn.Linear(hidden_size, hidden_size)   # [ADDED]
        self.k_proj = nn.Linear(hidden_size, hidden_size)   # [ADDED]
        self.v_proj = nn.Linear(hidden_size, hidden_size)   # [ADDED]
        ...
        # [MOD] 将 patch 网格与 image_size 传给每个 block（PRoPE 需要）
        self.blocks = nn.ModuleList([
            CDiTBlock(
                hidden_size, num_heads, mlp_ratio=mlp_ratio,
                patches_x=self.num_patches_x,               # [ADDED]
                patches_y=self.num_patches_y,               # [ADDED]
                image_size=self.input_size,                 # [ADDED]
    ...
    def forward(self, x, t, y, x_cond, rel_t, x_sup, viewmats, Ks=None):
        ...
        viewmats: (N, S+1, 4,4) 约定顺序 [contexts..., target]
        Ks:       (N, S+1, 3,3) 或 None
        """
