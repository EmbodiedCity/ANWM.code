...
class BaseDataset(Dataset):
    def __init__(
        ...
        all_data_config = load_yaml("real/config/data_config.yaml")
        ...
        for key in all_data_config["action_stats"]:
            self.ACTION_STATS[key] = np.expand_dims(
                all_data_config["action_stats"][key], axis=0
            )
    ...
    def _load_index(self, predefined_index) -> None:
        ...
        if predefined_index:
            ...
            print(
                f"Saved index to {index_to_data_path}, total samples: {len(self.index_to_data)}"
            )
    ...
    def _compute_projected_image(self, traj_data, curr_time, goal_time, rgb_img):
        ...
        projected_images = self.generate_augmented_image(
            K=K,
            depth_map=depth_map,
            rgb_img=rgb_img,
            pose_src=pose_src,
    ...
    def generate_augmented_image(
        self, K, depth_map, rgb_img, pose_src, pose_dst
    ) -> np.ndarray:
        """
        ...
        points_3d, colors = reproject_depth_to_other_pose_2seq(
            K, depth_map, rgb_img, pose_src, pose_dst
        )
        images = project_to_2d_image_2seq(
            K, points_3d, colors, image_size
        )  # (H, W, 3, goal_time)
        return images
    ...
    def _compute_actions(self, traj_data, curr_time, goal_time, rgb_img):
        ...
        projected_images = self._compute_projected_image(
            traj_data, curr_time, goal_time, rgb_img
        )
...
class TrainingDataset(BaseDataset):
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            # Compute actions
            _, goal_pos, projected_images = self._compute_actions(
                curr_traj_data, curr_time, goal_time, rgb_img
            )
            goal_pos[:, :3] = normalize_data(goal_pos[:, :3], self.ACTION_STATS)

            projected_tensor_list = [
                self.transform(Image.fromarray(img)) for img in projected_images
            ]
...
class EvalDataset(BaseDataset):
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            rgb_img = cv2.cvtColor(rgb_img, cv2.COLOR_BGR2RGB)
            # Compute actions
            actions, _, projected_images = self._compute_actions(
                curr_traj_data, curr_time, np.array(pred_times), rgb_img
            ...
            projected_tensor = torch.stack(projected_tensor_list, dim=0)
            print(
                f"Index {i}, projected_images shape: {projected_images.shape}, projected_tensor shape: {projected_tensor.size()}"
            )
            # # ===================== 保存图像 =====================
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            goal_image = self.transform(
                Image.open(get_data_path(self.data_folder, f_goal, goal_time))
            ).unsqueeze(0)
            curr_traj_data = self._get_trajectory(f_curr)
            ...
            actions, goal_pos, projected_images = self._compute_actions(
                curr_traj_data, curr_time, np.array([goal_time]), rgb_img
            )
