...
from real.dataset import TrajectoryEvalDataset
...
data_config = load_yaml("real/config/data_config.yaml")
...
def load_runtime_dependencies():
    global AutoencoderKL, PoseTrajectory3D, PoseRelation
    global lpips, main_ape, main_rpe, metrics, sync
...
def save_metric_to_disk(metric_logger, output_path):
    metric_logger.synchronize_between_processes()
    stats = {
        key: float(meter.global_avg) for key, meter in metric_logger.meters.items()
    }
    with open(output_path, "w", encoding="utf-8") as output_file:
        json.dump(stats, output_file, indent=2)


def downsample_absolute_actions(
    actions: torch.Tensor, target_steps: int
) -> torch.Tensor:
    """Match absolute GT waypoint actions to the candidate step count via delta aggregation."""
    if actions.ndim != 2:
        raise ValueError(
            f"Expected actions with shape [T, D], got {tuple(actions.shape)}"
        )
    if target_steps <= 0:
    ...
    if num_steps == target_steps:
        return actions
    if num_steps % target_steps != 0:
        raise ValueError(
            f"Cannot downsample GT actions with {num_steps} steps to {target_steps} candidate steps"
        )

    stride = num_steps // target_steps
    origin = torch.zeros(
        (1, actions.shape[-1]), dtype=actions.dtype, device=actions.device
    )
    abs_with_origin = torch.cat([origin, actions], dim=0)
    deltas = abs_with_origin[1:] - abs_with_origin[:-1]
    deltas = deltas.reshape(target_steps, stride, actions.shape[-1]).sum(dim=1)
...
def _stack_images_grid(
    img_paths, cols=4, unit_h=360, pad=12, bg=(255, 255, 255), caption=True
):
    """
    将若干图片按网格拼接：
      - cols: 每行列数
    ...
    for idx, im in enumerate(imgs):
        ...
        if cap_h and font is not None:
            ...
            if cap:
                draw.text(
                    (x0 + (im.size[0] - tw) // 2, y0 + unit_h + (cap_h - th) // 2),
                    cap,
                    fill=(0, 0, 0),
                    font=font,
                )
...
def plot_batch_final(
    init_imgs, pred_imgs, goal_imgs, idxs, losses, save_path="final_plan.png"
):
    # images are (B, c, h, w)
...
def plot_batch_trajectories(
    init_imgs, pred_imgs_seq, goal_imgs, idxs, save_path="trajectory_grid.png"
):
    """
...
class WM_Planning_Evaluator:
    def __init__(self, args):
        ...
        self.config = load_runtime_config(
            self.exp_eval,
            defaults="real/config/eval_config.yaml",
        )
        ...
        if self.args.output_dir is not None:
            ...
            os.makedirs(
                self.args.save_output_dir, exist_ok=True
            )  # 所有 rank 都创建目录（os.makedirs 是安全的）
        else:
        ...
        for dataset_name in self.dataset_names:
            ...
            print(
                f"Loaded {len(self.sampled_trajectories)} pre-sampled candidate trajectories from {candidate_traj_path}"
            )
    ...
    def _save_editor_run(
        ...
        if tag != "final":
            run_dir = _ensure_dir(
                os.path.join(run_dir, "candidates", f"cand_{cand_rank:03d}")
            )
        ...
        init_loss = float(
            self.loss_fn(
                init_img[None].to(self.device), goal_img[None].to(self.device)
            ).item()
        )
        step_losses = []
        for t in range(T):
            step_losses.append(
                float(
                    self.loss_fn(
                        step_seq[t : t + 1].to(self.device),
                        goal_img[None].to(self.device),
                    ).item()
                )
            )
    ...
    def save_single_sample_panel(
        ...
        if goal_xy is not None:
            ...
            gz = (
                float(-goal_xy[2])
                if (
                    isinstance(goal_xy, (list, tuple, np.ndarray)) and len(goal_xy) >= 3
                )
                else 0.0
            )
    ...
    def generate_actions(
        ...
        for traj in range(n_evals):
            ...
            candidate_steps = int(candidate_trajectories.shape[1] - 1)
            gt_actions_matched = downsample_absolute_actions(
                gt_actions[traj], candidate_steps
            )
            aligned_gt_actions.append(gt_actions_matched)
            ...
            gt_xyz_waypoint = (
                gt_actions_matched[:, :3].to("cpu").numpy()
            )  # [T, 3] waypoint单位，绝对位置
            # 添加起点 (0,0,0)
            gt_xyz_waypoint = np.concatenate(
                [np.zeros((1, 3), dtype=np.float32), gt_xyz_waypoint], axis=0
            )  # [T+1, 3] waypoint单位
            gt_xyz_meters = gt_xyz_waypoint * spacing
            num_gt_steps = gt_xyz_meters.shape[0]
            ...
            deltas_world = torch.tensor(
                candidate_deltas, dtype=torch.float32, device=self.device
            )  # (N, T, 4)
            ...
            cur_obs_image = (
                obs_image[traj].unsqueeze(0).repeat(self.num_samples, 1, 1, 1, 1)
            )
            cur_goal_image = (
                goal_image[traj]
                .unsqueeze(0)
                .repeat(self.num_samples, 1, 1, 1, 1)
                .squeeze(1)
            )
            cur_aug_image = (
                aug_image[traj].unsqueeze(0).repeat(self.num_samples, 1, 1, 1, 1)
            )
            ...
            if self.num_repeat_eval * self.num_samples > 12:
                ...
                for r in range(self.num_repeat_eval):
                    preds = self.autoregressive_rollout(
                        ...
                        aug_image=cur_aug_image,
                    )
                    preds = preds[:, -1]
                    loss = self.loss_fn(
                        preds.to(self.device), cur_goal_image.to(self.device)
                    ).flatten(0)
                    cur_losses.append(loss)
                loss = torch.stack(cur_losses).mean(dim=0)
            else:
                expanded_deltas = deltas_model.repeat(self.num_repeat_eval, 1, 1)
                expanded_obs_image = cur_obs_image.repeat(
                    self.num_repeat_eval, 1, 1, 1, 1
                )
                expanded_goal_image = cur_goal_image.repeat(
                    self.num_repeat_eval, 1, 1, 1
                )
                ...
                preds = self.autoregressive_rollout(
                    ...
                    expanded_deltas,
                    self.args.rollout_stride,
                    aug_image=expanded_aug,
                )
                preds = preds[:, -1]

                loss = self.loss_fn(
                    preds.to(self.device), expanded_goal_image.to(self.device)
                ).flatten(0)
            ...
            try:
                ...
                cand_dw_list = [
                    deltas_world[i].detach().cpu().numpy() for i in panel_ids
                ]  # 米单位
                cand_loss_list = [float(loss[i].item()) for i in panel_ids]
                # 直接使用 candidate_trajectories 的轨迹点进行可视化（米单位）
                cand_traj_points_list = [
                    candidate_trajectories[cid, :, :3] for cid in panel_ids
                ]  # [ (T+1,3) x K ] 米单位
                ...
                self.save_single_sample_panel(
                    ...
                    out_path=os.path.join(
                        image_plot_dir, f"sample_panel_idx{traj_id}.png"
                    ),
            ...
            gt_traj_this = self.actions_to_traj(gt_actions_matched[:, :3])
            ...
            ape_tensor = torch.full(
                (self.num_samples,), float("inf"), device=loss.device
            )
            ...
            if self.args.save_preds:
                ...
                for cid in range(self.num_samples):
                    ...
                    cand_traj = self.actions_to_traj(cand_actions)
                    cand_ate, cand_rpe_trans, cand_rpe_rot = self.eval_metrics(
                        gt_traj_this, cand_traj
                    )
                ...
                ape_sorted = sorted(
                    [
                        (cid, all_candidate_metrics[cid]["ape"])
                        for cid in range(self.num_samples)
                    ],
                    key=lambda x: x[1],
                )
                ...
                sample_detail = {
                    ...
                    "model_selection": {
                        ...
                        "selected_rank": int(
                            torch.where(sorted_idx == best_idx)[0].item()
                        ),
                    },
                    "oracle_selection": {
                        ...
                        "oracle_rank": int(
                            torch.where(sorted_idx == min_ape_idx)[0].item()
                        )
                        if min_ape_idx in sorted_idx
                        else -1,
                    },
                    "noisy_gt": {
                        ...
                        "rank": int(torch.where(sorted_idx == noisy_idx)[0].item())
                        if noisy_idx in sorted_idx
                        else -1,
                    },
                    "consistency": {
                        "agree_minloss_minape": int(agree_minloss_minape),
                        "loss_rank_of_oracle": int(
                            torch.where(sorted_idx == min_ape_idx)[0].item()
                        )
                        if min_ape_idx in sorted_idx
                        else -1,
            ...
            if self.args.save_preds:
                ...
                run_cands_dir = os.path.join(
                    editor_base_dir, "editor", f"run_{sid:03d}", "candidates"
                )
        ...
        aligned_gt_actions_tensor = torch.stack(aligned_gt_actions, dim=0)

        # Final rollout for selected deltas
        final_deltas = torch.cat(all_deltas, dim=0)
        preds = self.autoregressive_rollout(
            obs_image, final_deltas, self.args.rollout_stride, aug_image=aug_image
        )
        preds_completed = preds
        preds = preds[:, -1]

        loss = self.loss_fn(
            preds.to(self.device), goal_image.squeeze(1).to(self.device)
        ).flatten(0)
        ...
        sr_tensor = torch.tensor(
            sr_flags_all, dtype=torch.float32
        )  # [SR NEW] 每样本 0/1
        ...
        return (
            ...
            aligned_gt_actions_tensor,
            agree_tensor,
            selected_apes_tensor,
            oracle_min_apes_tensor,
            sr_tensor,
        )
    ...
    def visualize_trajectories(
        ...
        log_viz_single(
            ...
            metric_waypoint_spacing=data_config[dataset_name][
    ...
    def make_paper_panel_for_dataset(
        self,
        dataset_name: str,
        ...
        if not cand:
            raise RuntimeError(
                f"[panel] no images matched in {plots_dir} with patterns={patterns} and indices={only_indices}"
            )

        panel = _stack_images_grid(
            cand, cols=cols, unit_h=unit_h, pad=pad, caption=caption
        )
        out_path = os.path.join(base_dir, out_name)
        panel.save(out_path)
        print(
            f"[panel saved] {out_path}  (images={len(cand)}, cols={cols}, unit_h={unit_h})"
        )
    ...
    def evaluate(self):
        for dataset_name in self.dataset_names:
            ...
            if self.args.save_preds:
                dataset_save_output_dir = os.path.join(
                    self.args.save_output_dir, dataset_name
                )
                os.makedirs(dataset_save_output_dir, exist_ok=True)
                eval_save_output_dir = os.path.join(
                    dataset_save_output_dir, self.eval_name
                )
            ...
            for (
                ...
                for i in range(len(obs_image)):
                    ...
                    gt_traj_i = self.actions_to_traj(aligned_gt_actions[i, :, :3])
                    ...
                    metric_logger.meters[f"{dataset_name}_rpe_trans"].update(
                        rpe_trans, n=1
                    )
                    metric_logger.meters[f"{dataset_name}_pos_diff_norm"].update(
                        pos_diff_norm, n=1
                    )
                    metric_logger.meters[f"{dataset_name}_yaw_diff_norm"].update(
                        yaw_diff_norm, n=1
                    )

                    metric_logger.meters[f"{dataset_name}_agree_minloss_minape"].update(
                        float(agree_flags[i].item()), n=1
                    )
                    metric_logger.meters[f"{dataset_name}_selected_ape"].update(
                        float(selected_apes[i].item()), n=1
                    )
                    metric_logger.meters[f"{dataset_name}_oracle_min_ape"].update(
                        float(oracle_min_apes[i].item()), n=1
                    )
                    metric_logger.meters[f"{dataset_name}_sr"].update(
                        float(sr_tensor[i].item()), n=1
                    )  # [SR NEW]
            ...
            output_fn = os.path.join(
                self.args.save_output_dir, f"{dataset_name}_{self.eval_name}.json"
            )
...
def cli():
    ...
    parser.add_argument(
        "--datasets", type=str, default="sekai_new", help="dataset name"
    )
    parser.add_argument(
        "--output_dir", type=str, default="outputs/planning", help="output directory"
    )
    parser.add_argument(
        "--save_preds",
        action="store_true",
        default=False,
        help="whether to save prediction tensors or not",
    )
    ...
    parser.add_argument(
        "--num_samples", type=int, default=5, help="num nomad samples to predict"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="top k samples to take mean and var for RULE",
    )
    parser.add_argument(
        "--opt_steps", type=int, default=15, help="num iterations for RULE"
    )
    parser.add_argument(
        "--num_repeat_eval", type=int, default=1, help="number of evals for one action"
    )
