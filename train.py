...
def main(args):
    ...
    os.makedirs(
        config["results_dir"], exist_ok=True
    )  # Make results folder (holds all experiment subfolders)
    experiment_dir = (
        f"{config['results_dir']}/{config['run_name']}"  # Create an experiment folder
    )
    ...
    if os.path.isfile(latest_path) or config.get("from_checkpoint", 0):
        ...
        latest_path = (
            ...
            else config.get("from_checkpoint", 0)
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, rel_t, aug in loader:
            ...
            with torch.amp.autocast(
                "cuda", enabled=bfloat_enable, dtype=torch.bfloat16
            ):
                with torch.no_grad():
            ...
            if not bfloat_enable:
                ...
                if config.get("grad_clip_val", 0) > 0:
                    ...
                    torch.nn.utils.clip_grad_norm_(
                        model.parameters(), max_norm=config["grad_clip_val"]
                    )
                scaler.step(opt)
                scaler.update()
            ...
            if train_steps % args.log_every == 0:
                ...
                samples_per_sec = (
                    dist.get_world_size() * x_cond.shape[0] * steps_per_sec
                )
