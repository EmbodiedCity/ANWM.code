...
class ActionEmbedder(nn.Module):
    ...
    def __init__(self, hidden_size, frequency_embedding_size=256):
        ...
        self.angle_emb = TimestepEmbedder(
            hidden_size - 3 * hsize, frequency_embedding_size
        )
...
class CDiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(), nn.Linear(hidden_size, 14 * hidden_size, bias=True)
        )
    ...
    def forward(self, x, c, x_cond, x_sup):
        (
            ...
            scale_mlp,
            gate_mlp,
        ) = self.adaLN_modulation(c).chunk(14, dim=1)
        x = x + gate_msa.unsqueeze(1) * self.attn(
            modulate(self.norm1(x), shift_msa, scale_msa)
        )
        x_ca_norm = modulate(self.norm2(x), shift_ca_x, scale_ca_x)
        x_sup_norm = modulate(self.norm_sup(x_sup), shift_xsup, scale_xsup)
        x = (
            x
            + gate_ca_xsup.unsqueeze(1)
            * self.cttn(
                query=x_ca_norm, key=x_sup_norm, value=x_sup_norm, need_weights=False
            )[0]
        )
        ...
        x = (
            ...
            * self.cttn(
                query=x_ca_norm, key=x_cond_norm, value=x_cond_norm, need_weights=False
            )[0]
        )
...
class FinalLayer(nn.Module):
    ...
    def forward(self, x, c, x_supervised_token):
        ...
        attn_output = self.attn(
            query=x, key=x_supervised_token, value=x_supervised_token
        )
...
class CDiT(nn.Module):
    ...
    def forward(self, x, t, y, x_cond, rel_t, x_supervised):
        ...
        x_supervised = (
            self.x_embedder(x_supervised) + self.pos_embed[self.context_size :]
        )
        y = self.y_embedder(y)
