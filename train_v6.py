    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, rel_t, aug in loader:
            ...
            with torch.amp.autocast('cuda', enabled=bfloat_enable, dtype=torch.bfloat16):
                ...
                xy_cond = aug.unsqueeze(2).flatten(0, 1)                                                                                     # [B*num_goals, 1, 4, 28, 28]
                x_cond = torch.cat((x_cond, xy_cond), dim=1)                    # [B*num_goals, num_cond+1, 4, 28, 28]                      
               #  print(f"x_cond shape: {x_cond.size()}")
                y = y.flatten(0, 1)
                rel_t = rel_t.flatten(0, 1)
                
                t = torch.randint(0, diffusion.num_timesteps, (x_start.shape[0],), device=device)
                model_kwargs = dict(y=y, x_cond=x_cond, rel_t=rel_t, x_sup=xy_cond.squeeze(1))
