...
def plot_traj3d(pred_xyz: torch.Tensor,
    ...
    # 转换为 float32 以支持 bfloat16 类型
    pred = pred_xyz.detach().cpu().to(torch.float32).numpy()
    x, y, z = pred[:, 0], pred[:, 1], pred[:, 2]

    if gt_xyz is not None:
        gt_np = gt_xyz.detach().cpu().to(torch.float32).numpy()
    ...
    if yaw_seq is not None:
        yaw_np = yaw_seq.detach().cpu().to(torch.float32).numpy()
...
class WM_Planning_Evaluator:
    def __init__(self, args):
        ...
        # NEW: 直接在本脚本中初始化分布式进程组
        _gpu_id = _init_pg()  # NEW
    ...
    def _optimize_single_traj(
        self, obs_img_1, goal_img_1, T, aug_img_1, steps, lr=0.05, smooth_w=1e-3,
    ...
    # ================================================================
    def _energy(
        self, obs_img, goal_img, deltas_norm, aug_img,
    ...
    # 概率场采样：SGLD
    # ================================================================
    def _sample_langevin(self, obs_img_1, goal_img_1, T, aug_img_1,
            ...
            if self.args.sampler == "langevin":
                ...
                with torch.no_grad():
                    preds_seq_agg = self.autoregressive_rollout(
                        obs_1, deltas_agg, self.args.rollout_stride, aug_image=aug_1
                    )
            ...
            for (idxs, obs_image, goal_image, gt_actions, goal_pos, aug_image) in metric_logger.log_every(curr_data_loader, 1, header):
                ...
                with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
                    pred_actions, pred_yaw = self.generate_actions(
                        ...
                        self.config["trajectory_eval_len_traj_pred"], aug_image=aug_image
                    )
                ...
                for i in range(len(obs_image)):
                    ...
                    # 计算成功率（基于位置误差阈值）
                    success = 1.0 if pos_diff_norm <= self.args.success_threshold else 0.0
                    metric_logger.meters['{}_sr'.format(dataset_name)].update(success, n=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--success_threshold", type=float, default=5.0, help="success threshold in meters (position error <= threshold is considered success)")
